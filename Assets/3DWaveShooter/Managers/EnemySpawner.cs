...
public class EnemySpawner : MonoBehaviour
{
    ...
    {
        if (GameManager.inst.dartReplaceEnabled
        && enemyObject == GameManager.inst.fastEnemyPrefab)
        {
            // spawn a dart at 'pos'
            GameObject go = Pool.Spawn(GameManager.inst.dartEnemyType, pos, Quaternion.identity);
            var e = go.GetComponent<Enemy>();
            e.Initialize();
            e.moveSpeed *= GameManager.inst.dartSpeedMultiplier;
            foreach (var mr in go.GetComponentsInChildren<MeshRenderer>())
                for (int i = 0; i < mr.materials.Length; i++)
                    if (mr.materials[i].HasProperty("_Color"))
                        mr.materials[i].color = GameManager.inst.dartColor;

            curEnemies.Add(go);
            return;  // skip the normal spawn
        }
